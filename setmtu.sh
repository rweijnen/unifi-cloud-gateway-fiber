...
MTU_ETH4=$(jq '.[] | select(.identification.id == "eth6") | .status.mtu' "$INTERFACES_FILE")
if [[ "$MTU_ETH4" -ne 1512 ]]; then
    jq 'map(if .identification.id == "eth6" then .status.mtu = 1512 else . end)' "$INTERFACES_FILE" > "$TEMP_FILE" && mv -f "$TEMP_FILE" "$INTERFACES_FILE"
    log "Changed MTU for eth4 to 1512"
    MODIFIED=true
fi

# Modify MTU for eth6.6
log "Checking current MTU for eth6.6"
MTU_ETH4_6=$(jq '.[] | select(.identification.id == "eth6.6") | .status.mtu' "$INTERFACES_FILE")
if [[ "$MTU_ETH4_6" -ne 1508 ]]; then
    jq 'map(if .identification.id == "eth6.6" then .status.mtu = 1508 else . end)' "$INTERFACES_FILE" > "$TEMP_FILE" && mv -f "$TEMP_FILE" "$INTERFACES_FILE"
    log "Changed MTU for eth6.6 to 1508"
    MODIFIED=true
fi
